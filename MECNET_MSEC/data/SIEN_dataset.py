...
class DatasetFromFolder(data.Dataset):
    ...
    rot = None
    """
    def __init__(self, upscale_factor, data_augmentation, group_file, patch_size, black_edges_crop, hflip, rot, resize,transform=transform()):
    ...
    def __getitem__(self, index):
        ...
        if self.resize:
            if inputs.shape[0] >= inputs.shape[1]:
                inputs = cv2.transpose(inputs)
                target = cv2.transpose(target)

            image_size = 512
            image_size_w = 512
            inputs = cv2.resize(inputs, (image_size, image_size_w))
            target = cv2.resize(target, (image_size, image_size_w))
