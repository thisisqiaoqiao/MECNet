...
def create_dataset(opt,dataset_opt):
    ...
    if mode == 'UEN_train':
        from .SIEN_dataset import DatasetFromFolder as D

        dataset = D(upscale_factor=opt['scale'], data_augmentation=dataset_opt['augment'],
                    group_file=dataset_opt['filelist'],
                    patch_size=dataset_opt['IN_size'], black_edges_crop=False, hflip=True, rot=True,resize=False)

    elif mode == 'UEN_val':
        from .SIEN_dataset import DatasetFromFolder as D
        dataset = D(upscale_factor=opt['scale'], data_augmentation=False,
                    group_file=dataset_opt['filelist'],
                    patch_size=None, black_edges_crop=False, hflip=False, rot=False,resize=False)
